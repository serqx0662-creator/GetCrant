...
export default function SectionHeader({ title, subtitle, onPrev, onNext }: SectionHeaderProps) {
  return (
    <div className="flex items-start justify-between mb-6">
      ...
      <div className="flex items-center gap-2 mt-1 md:flex">
        <button
          onClick={onPrev}
          className="w-10 h-10 flex items-center justify-center rounded-lg border border-[#D0D5DD] bg-white hover:bg-gray-50 transition-colors text-[#1D2939]"
          aria-label="Назад"
        >
          <ChevronLeft size={16} />
        </button>
